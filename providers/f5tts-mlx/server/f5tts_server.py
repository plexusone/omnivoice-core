...
import localtts_pb2 as pb
import localtts_pb2_grpc as pb_grpc
...
class F5TTSServicer(pb_grpc.LocalTTSServicer):
    ...
    def _load_model(self, model_path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            # Import generate function - model loading happens on first call
            from f5_tts_mlx.generate import generate as f5_generate

            # Store the generate function for later use
            self._generate_fn = f5_generate

            # Warm up by doing a minimal generation to load the model
            # This ensures the model is in memory
            logger.info("Warming up model with test generation...")
            _ = f5_generate(
                generation_text="test",
                steps=1,  # Minimal steps for fast warmup
            )
    ...
    def _synthesize_audio(
        ...
        # Generate audio using the f5_tts_mlx generate function
        # The generate function works with file paths, so we need temp files
        ref_audio_path = None

        try:
            if reference_audio is not None and reference_text is not None:
                # Voice cloning mode - write reference audio to temp file
...
def serve(socket_path: str, auto_load: bool = False):
    ...
    pb_grpc.add_LocalTTSServicer_to_server(
